// @ts-check
// versioned extractor id + ddl strings (kept in js for programmatic bootstrap if you want it later)

const EXTRACTOR_VERSION = 'indexer-v1';

const RECIPE_SCHEMA_VERSION = 1;

// search_document is plain tsvector; filled on insert in db.js (generated columns + gin were picky across pg versions)
...
CREATE INDEX IF NOT EXISTS idx_recipes_ingredient_tokens_gin
  ON recipes USING GIN (ingredient_tokens);
CREATE INDEX IF NOT EXISTS idx_recipes_appliance_tags_gin
  ON recipes USING GIN (appliance_tags);
CREATE INDEX IF NOT EXISTS idx_recipes_dietary_tags_gin
...
CREATE INDEX IF NOT EXISTS idx_recipes_dietary_total
  ON recipes USING BTREE (dietary_tags, total_minutes);

CREATE INDEX IF NOT EXISTS idx_recipe_ingredients_name
  ON recipe_ingredients USING BTREE (normalized_name);
`;
