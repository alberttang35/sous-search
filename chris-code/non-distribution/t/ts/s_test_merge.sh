...
files=("$T_FOLDER"/ts/s_inputs/s_merge{1..2}.txt)
...
if DIFF_PERCENT=$DIFF_PERCENT t/gi-diff.js <(sort d/global-index.txt) <(sort "$T_FOLDER"/ts/s_inputs/s_merge_output.txt) >&2;
then
